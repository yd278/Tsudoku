            ...
            if same_row{
                for shift in 0..3{
                    if box_id_p/3*3+shift != px{
                        res.extend(Self::intersect(House::Row(box_id_p/3*3+shift), House::Box(box_id_p)));
                    }
                }
            }else if same_col{
                for shift in 0..3{
                    if box_id_p%3*3+shift != py{
                        res.extend( Self::intersect(House::Col(box_id_p%3*3+shift), House::Box(box_id_p)));
                    }
                }
            } else{
                res.extend(Self::box_coords(box_id_p).filter(|&(x,y)|{
                    !((x==px && y==py)||(x==qx && y == qy))
                }))
            }
...
mod test_coord{
    ...
    fn test_pincer_same_row_box(){
        ...
        let exp = [(4,0),(4,1),(4,2),(4,3),(4,6),(4,7),(4,8),(3,3),(3,4),(3,5),(5,3),(5,4),(5,5)];
        assert_eq!(res.len(), exp.len());
        for i in 0..res.len(){
            assert_eq!(res[i],exp[i])
        }
    ...
    fn test_pincer_same_col_box(){
        ...
        let exp = [(0,4),(1,4),(2,4),(3,4),(6,4),(7,4),(8,4),(3,3),(4,3),(5,3),(3,5),(4,5),(5,5)];
        assert_eq!(res.len(), exp.len());
        for i in 0..res.len(){
            assert_eq!(res[i],exp[i])
        }
